...
void Widget::initializeGL()
{
    ...
    m_objects[m_objects.size() - 1]->loadObjectFromFile(":/mesh/capsule.obj");
    m_transformObjects.append(m_objects[m_objects.size() - 1]);

    m_groups[0]->addObject(m_camera);

    Material *mtl = new Material;
    mtl->setDiffuseMap(":/skybox2.png");
    mtl->setShinnes(96);
    mtl->setDiffuseColor({1.0,1.0,1.0});
    mtl->setAmbienceColor({1.0,1.0,1.0});
    mtl->setSpecularColor({1.0,1.0,1.0});
    m_skyBox = new SkyBox(100, mtl);
...
void Widget::initCube(float width)
{
    ...
    Material *mtl = new Material;
    mtl->setDiffuseMap(":/cube.jpeg");
    mtl->setShinnes(96);
    mtl->setDiffuseColor({1.0,1.0,1.0});
    mtl->setAmbienceColor({1.0,1.0,1.0});
    mtl->setSpecularColor({1.0,1.0,1.0});

    ObjectEngine3D *obj = new ObjectEngine3D;
    obj->addObject(new SimpleObject3D(vertexes, indexex, mtl));
