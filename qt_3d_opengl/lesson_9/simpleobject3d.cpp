...
void SimpleObject3D::init(const QVector<VertexData> &vertData, const QVector<GLuint> &indexes, Material *material)
{
    ...
    m_material = material;
    if (material->getIsUsingDiffuseMap())
    {
...
void SimpleObject3D::draw(QOpenGLShaderProgram *program, QOpenGLFunctions *functions)
{
    ...
    program->setUniformValue("u_materialProperty.sinnes", m_material->shinnes());
    program->setUniformValue("u_isUsingDiffuseMap", m_material->getIsUsingDiffuseMap());
